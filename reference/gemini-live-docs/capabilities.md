...
| **[Proactive audio](https://ai.google.dev/gemini-api/docs/live-api/capabilities#proactive-audio)** | Not supported | Supported. When enabled, the model can proactively decide not to respond if the input content is not relevant. Set `proactive_audio` to `true` in the `proactivity` config (requires `v1beta`). |
| **[Affective dialogue](https://ai.google.dev/gemini-api/docs/live-api/capabilities#affective-dialog)** | Not supported | Supported. The model adapts its response style to match the expression and tone of the input. Set `enable_affective_dialog` to `true` in session config (requires `v1beta`). |
...
To use affective dialog, set the api version to `v1beta` and set
...
To use it, set the api version to `v1beta` and configure the `proactivity`
